...
const generateCard = () => {
    const breadcrumbList = document.querySelector('.breadcrumb__list'),
        goodWrapper = document.querySelector('.good.wrapper'),
        title = document.querySelector('title');

    const renderCard = data => {
            title.textContent = `${data.name} - IKEA`;
            const listNav = `
            <li class="breadcrumb__list-item">
                <a href="goods.html?cat=${data.category}" class="breadcrumb__link"><span>${data.category}</span></a>
            </li>

            <li class="breadcrumb__list-item">
                <a href="goods.html?subcat=${data.subcategory}" class="breadcrumb__link"><span>${data.subcategory}</span></a>
            </li>

            <li class="breadcrumb__list-item">
                <a href="#" class="breadcrumb__link"><span>${data.name}</span></a>
            </li>
        `;

            breadcrumbList.innerHTML = listNav;
            ...
            <div class="good-item${data.count === 0 ? ' not-in-stock' : ''}"> <!-- not-in-stock - нет в наличии-->
                <p class="good-item__new" ${data.count >= NEW_COUNT_ITEM ? 'style="display: block"' : ''}>Новинка</p>

                <div class="good-item__section">
                    <div>
                        <h2 class="good-item__header">${data.name}</h2>
                        <p class="good-item__description">${data.description}</p>

                    </div>
                    <p class="good-item__price">
                        <span class="good-item__empty">Нет в наличии</span>
                        <span class="good-item__price-value">${data.price}</span>
                        <span class="good-item__currency">₽</span>
                    </p>

                </div>
                <div class="good-item__buttons">
                    <button type="button" class="btn btn-good" data-idd="${data.id}">
                    ...
                    <button type="button" class="btn btn-add-wishlist" data-idd="${data.id}">
        ...
        `;
            ...
            goodWrapper.innerHTML = card;
            checkWishlist(data.id);

            document.body.addEventListener('click', event => {
                if (event.target.closest('.btn-add-wishlist')) {
                    userData.wishList = data.id;
                    checkWishlist(data.id);
                }

                if (event.target.closest('.btn-good')) {
                    userData.cartList = data.id;
                }
            });
        },
        checkWishlist = id => {
